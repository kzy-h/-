        ...
        >
          <div class="pet-pose h-full w-full origin-bottom" :style="avatarStyles">
            ...
            <ImageCrossFade
              v-show="!live2dActive"
              ref="imageFadeRef"
              class="h-full w-full drop-shadow-[0_12px_10px_rgba(0,0,0,0.28)]"
              :class="avatarMotionClasses"
              :src="targetAvatarUrl"
              :duration="smoothTransitionsEnabled ? 280 : 0"
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      ...
      actionFile?: string;
      actionMotion?: PetMotionPreset;
      smoothTransitionsEnabled?: boolean;
      subtleMotionEnabled?: boolean;
    }>(),
    {
      actionFile: "",
      actionMotion: "breathe",
  ...
  const avatarMotionClasses = computed(() => {
    if (props.subtleMotionEnabled === false) return [];
    const motion = props.actionFile ? (props.actionMotion ?? "breathe") : "breathe";
    return motion === "none" ? [] : [`pet-motion-${motion}`];
  });
  ...
  const preloadedActionUrls = new Set<string>();

  async function preloadActionImages() {
    if (props.smoothTransitionsEnabled === false) return;
    const r = role.value;
    const files = [...new Set(Object.values(PET_ACTIONS).map((action) => action.file))];
    await Promise.allSettled(
      files.map(async (actionFile) => {
        const path = await invoke<string>("get_pet_action_file", {
          characterFolder: r.character_folder,
          actionFile,
        });
        const url = await toAvatarUrl(path);
        if (preloadedActionUrls.has(url)) return;
        const image = new Image();
        await new Promise<void>((resolve, reject) => {
          image.onload = () => resolve();
          image.onerror = () => reject(new Error(`无法预加载动作立绘: ${actionFile}`));
          image.src = url;
        });
        await image.decode().catch(() => {});
        preloadedActionUrls.add(url);
      })
    );
  }

  onMounted(() => void preloadActionImages());

  watch(
    () => [role.value.roleId, role.value.character_folder, props.smoothTransitionsEnabled],
    () => void preloadActionImages()
  );
...
<style scoped>
  ...
  .pet-motion-breathe {
    animation: pet-idle-breathe 5.6s ease-in-out infinite;
    will-change: transform;
  }

  .pet-motion-sway {
    animation: pet-idle-sway 4.8s ease-in-out infinite;
    transform-origin: center bottom;
    will-change: transform;
  }

  .pet-motion-sleepy {
    animation: pet-idle-sleepy 6.2s ease-in-out infinite;
    transform-origin: center bottom;
    will-change: transform;
  }

  .pet-motion-bounce {
    animation: pet-idle-bounce 2.8s ease-in-out infinite;
    will-change: transform;
  }
